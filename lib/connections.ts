...
export type PersonalMessage={id:string;conversation_id:string;sender_id:string;body:string;read_at:string|null;created_at:string};
...
export async function personalMessages(conversationId:string):Promise<PersonalMessage[]>{requireSupabaseConfig();const {data,error}=await supabase.from('personal_messages').select('id,conversation_id,sender_id,body,read_at,created_at').eq('conversation_id',conversationId).order('created_at',{ascending:true});if(error)throw new Error(error.message);return (data??[]) as PersonalMessage[]}
