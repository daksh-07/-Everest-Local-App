...
export type AdminResource = { key: AdminResourceKey; label: string; table: string; select: string; searchColumn: string | null; orderColumn: string; description: string; sensitive?: boolean };
...
export const ADMIN_RESOURCES: AdminResource[] = [
  ...
  {key:'service_requests',label:'Service Requests',table:'service_requests',select:'id,customer_id,category_id,service_id,suburb,city,state,preferred_date,budget,status,delivery_mode,is_live,live_status,live_radius_km,live_radius_stage,live_started_at,live_expires_at,requested_arrival_window,created_at,updated_at',searchColumn:'suburb',orderColumn:'created_at',description:'Customer demand entering matching and quoting. Everest Live requests include real search status, radius stage and expiry.'},
  ...
  {key:'inventory',label:'Inventory',table:'inventory',select:'product_id,stock_quantity,reserved_quantity,low_stock_threshold,updated_at',searchColumn:null,orderColumn:'updated_at',description:'Stock and reservation state. Search is disabled because the available identifier is UUID-only.'},
  ...
  {key:'delivery_assignments',label:'Delivery Assignments',table:'delivery_assignments',select:'id,delivery_id,driver_id,assigned_at,accepted_at,completed_at',searchColumn:null,orderColumn:'assigned_at',description:'Driver assignment operations. Search is disabled because the available identifiers are UUID-only.'},
  ...
  {key:'conversations',label:'Messaging',table:'conversations',select:'id,customer_id,business_id,request_id,booking_id,quote_id,created_at',searchColumn:null,orderColumn:'created_at',description:'Conversation routing metadata; message content is not bulk-exported. Search is disabled because available identifiers are UUID-only.'},
  ...
  {key:'service_matches',label:'Service Matching',table:'service_matches',select:'id,request_id,business_id,score,reason,created_at',searchColumn:null,orderColumn:'created_at',description:'Business-request matching state. Search is disabled because the available identifiers are UUID-only.'},
  {key:'opportunities',label:'Opportunities',table:'opportunities',select:'id,request_id,business_id,status,is_live,approximate_distance_km,eta_seconds,notified_at,viewed_at,responded_at,expires_at,created_at',searchColumn:'status',orderColumn:'created_at',description:'Business opportunity distribution, including Everest Live delivery and response timestamps.'},
...
export async function getAdminResourcePage(resource: AdminResource, page: number, pageSize: number, search: string) {
  ...
  if (term && resource.searchColumn) query = query.ilike(resource.searchColumn, `%${term}%`);
