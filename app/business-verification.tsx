...
export default function BusinessVerification(){
  ...
  async function submit(){
    ...
    try{
      ...
      setMessage('Verification submitted\nYour business details are now awaiting Everest admin review.');
    }catch(e){
  ...
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.page}>
    ...
    {loading?<ActivityIndicator/>:business?<>
      <View style={s.card}>
        ...
        <Text style={s.meta}>Only server-authorized admin review can mark a business verified.</Text>
      </View>
      ...
      <Text style={s.note}>ABN submission is the first verification step. Everest admin may request supporting documents during review.</Text>

      <Pressable
        disabled={busy||!abnValid||business.verification_status==='PENDING'||business.verification_status==='VERIFIED'}
        onPress={()=>void submit()}
        style={[s.button,(busy||!abnValid||business.verification_status==='PENDING'||business.verification_status==='VERIFIED')?s.buttonDisabled:null]}
      >
        <Text style={s.buttonText}>{busy?'SUBMITTING…':'SUBMIT FOR REVIEW'}</Text>
      </Pressable>
    </>:<View style={s.empty}><Text style={s.name}>No business profile.</Text><Text style={s.meta}>Create one before starting verification.</Text></View>}
    {!!error&&<Text style={s.error}>{error}</Text>}
    {!!message&&<Text style={s.message}>{message}</Text>}
  </ScrollView></SafeAreaView>
}
const s=StyleSheet.create({safe:{flex:1,backgroundColor:'#f8f7f4'},page:{padding:20,paddingBottom:50},eyebrow:{fontSize:10,fontWeight:'900',letterSpacing:2,color:'#777'},title:{fontSize:31,fontWeight:'900',marginTop:7,marginBottom:22},card:{backgroundColor:'#151515',borderRadius:20,padding:20},name:{fontSize:17,fontWeight:'800',color:'#fff'},status:{fontSize:10,fontWeight:'900',letterSpacing:1,color:'#aaa',marginTop:8},meta:{fontSize:12,lineHeight:18,color:'#777',marginTop:6},label:{fontSize:9,fontWeight:'900',letterSpacing:1.2,color:'#777',marginTop:20,marginBottom:7},input:{height:52,borderRadius:14,borderWidth:1,borderColor:'#dfdcd5',backgroundColor:'#fff',paddingHorizontal:15},inputError:{borderColor:'#b42318'},inputValid:{borderColor:'#245b35'},note:{fontSize:11,color:'#777',lineHeight:17,marginTop:12},button:{height:52,borderRadius:14,backgroundColor:'#111',alignItems:'center',justifyContent:'center',marginTop:18},buttonDisabled:{opacity:0.45},buttonText:{color:'#fff',fontSize:10,fontWeight:'900'},empty:{backgroundColor:'#fff',borderRadius:19,padding:28,alignItems:'center'},error:{fontSize:12,color:'#b42318',marginTop:12},valid:{fontSize:12,color:'#245b35',marginTop:8},message:{fontSize:12,color:'#245b35',marginTop:12}});
