...
export default function DriverOnboarding() {
  ...
  useEffect(()=>{let active=true;(async()=>{try{
    const [{data:{user}}, profileResult, app, req]=await Promise.all([supabase.auth.getUser(),supabase.from('profiles').select('phone').eq('id',(await supabase.auth.getUser()).data.user?.id ?? '').maybeSingle(),getDriverApplication(),getDriverRequirements()]);
