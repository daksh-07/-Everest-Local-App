...
export default function DriverOnboarding() {
  ...
  useEffect(()=>{let active=true;(async()=>{try{
    const [{data:{user}}, app, req]=await Promise.all([supabase.auth.getUser(),getDriverApplication(),getDriverRequirements()]);
    if(!active)return; setEmail(user?.email ?? ''); setRequirements(req); setApplication(app);
