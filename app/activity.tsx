...
export default function Activity(){
 ...
 }catch(e){setError(e instanceof Error?e.message:'We could not load your activity.')}finally{setLoading(false)}}
 useEffect(()=>{void load()},[]);
 return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.page}><Text style={s.eyebrow}>EVEREST LOCAL</Text><Text style={s.title}>Activity</Text><Text style={s.copy}>Requests, quotes, bookings and orders in one place.</Text>{loading?<ActivityIndicator style={{marginTop:30}}/>:error?<View style={s.empty}><Text style={s.emptyTitle}>We couldn't load your activity.</Text><Text style={s.emptyCopy}>Please try again.</Text><Pressable onPress={()=>void load()} style={s.button}><Text style={s.buttonText}>RETRY</Text></Pressable></View>:items.length?items.map(item=><Pressable key={item.key} onPress={()=>router.push(item.route)} style={s.card}><View style={{flex:1}}><Text style={s.label}>{item.label}</Text><Text style={s.detail} numberOfLines={2}>{item.detail}</Text></View><View style={s.status}><Text style={s.statusText}>{item.status}</Text><Text style={s.arrow}>›</Text></View></Pressable>):<View style={s.empty}><Text style={s.emptyTitle}>Nothing here yet.</Text><Text style={s.emptyCopy}>Post a job, request a quote, book a service or buy a product and your activity will appear here.</Text><Pressable onPress={()=>router.push('/search')} style={s.button}><Text style={s.buttonText}>EXPLORE LOCAL</Text></Pressable></View>}<View style={s.links}><Pressable onPress={()=>router.push('/notifications')}><Text style={s.link}>Notifications</Text></Pressable><Pressable onPress={()=>router.push('/reviews')}><Text style={s.link}>Reviews</Text></Pressable></View></ScrollView></SafeAreaView>;
}
