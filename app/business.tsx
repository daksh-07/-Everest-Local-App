...
export default function Business(){
 const [name,setName]=useState('');const [description,setDescription]=useState('');const [abn,setAbn]=useState('');const [phone,setPhone]=useState('');const [email,setEmail]=useState('');const [suburb,setSuburb]=useState('');const [busy,setBusy]=useState(false);const [message,setMessage]=useState('');
 const fields:Field[]=[
 ...
 async function create(){setMessage('');if(!name.trim()||!suburb.trim()){setMessage('Business name and suburb are required.');return;}setBusy(true);try{const id=await createBusinessProfile({name,description,abn,phone,email,suburb,city:'Sydney',state:'NSW'});setMessage(`Business created successfully. Reference: ${id}. Submit verification before marketplace publication.`);}catch(e){setMessage(e instanceof Error?e.message:'Could not create business.')}finally{setBusy(false)}}
