...
const businessRoutes=new Set(['/business-dashboard','/business-verification','/business-orders','/products','/services','/service-areas','/opportunities']);
const adminRoutes=new Set(['/admin']);
const deliveryRoutes=new Set(['/delivery']);

export default function RootLayout(){
 const pathname=usePathname();const router=useRouter();const [ready,setReady]=useState(!supabaseConfigured);const [role,setRole]=useState<AppRole|null>(null);
 useEffect(()=>{
  if(!supabaseConfigured){setReady(true);return;}
  let active=true;
  const load=async()=>{
   const {data:{session}}=await supabase.auth.getSession();
   if(!session){if(active){setRole(null);setReady(true)}}
   else{
    const {data}=await supabase.from('profiles').select('role').eq('id',session.user.id).maybeSingle();
    if(active){setRole((data?.role as AppRole|undefined)??null);setReady(true)}
   }
  };
  void load();
