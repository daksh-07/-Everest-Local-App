...
export default function BusinessProfile() {
  ...
  async function askAboutService(service:Service){if(!business||growthBusy)return;setGrowthBusy('ask:'+service.id);setError('');setMessage('');try{const conversationId=await getOrCreateBusinessInquiry({businessId:business.id,contextType:'SERVICE',contextId:service.id});router.push(('/messages?marketId='+conversationId) as never)}catch(e){setError(e instanceof Error?e.message:'Could not open service chat.')}finally{setGrowthBusy('')}}
