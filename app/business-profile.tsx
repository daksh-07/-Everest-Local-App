...
import { setBusinessSaved } from '@/lib/marketplace';
...
export default function BusinessProfile() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if(user){const {data: savedRow,error:savedError}=await supabase.from('saved_businesses').select('business_id').eq('customer_id',user.id).eq('business_id',id).maybeSingle();if(savedError)throw savedError;setSaved(Boolean(savedRow));}else setSaved(false);
    } catch (e) {
  ...
  async function toggleSaved(){
    ...
    try{await setBusinessSaved(id,!saved);setSaved(value=>!value);setSaveMessage(saved?'Removed from saved.':'Saved to your account.');}
    catch(e){setSaveMessage(e instanceof Error?e.message:'Could not update saved businesses.');}
  ...
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.page}>
    ...
    <View style={s.hero}>
      ...
      <Pressable disabled={saveBusy} onPress={()=>void toggleSaved()} style={s.saveButton} accessibilityLabel={saved?'Remove business from saved':'Save business'}>{saveBusy?<ActivityIndicator size="small"/>:<><Ionicons name={saved?'bookmark':'bookmark-outline'} size={17} color="#fff"/><Text style={s.saveText}>{saved?'SAVED':'SAVE BUSINESS'}</Text></>}</Pressable>
    </View>
    {saveMessage?<Text style={s.saveMessage}>{saveMessage}</Text>:null}
