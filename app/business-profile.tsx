...
export default function BusinessProfile() {
  ...
  return <SafeAreaView style={s.safe}><ScrollView refreshControl={<RefreshControl refreshing={refreshing} enabled={!saving&&!followBusy} tintColor={colors.text} onRefresh={()=>void load(true)}/>} contentContainerStyle={s.page}>
    ...
    {services.length?services.map(item=><Pressable accessibilityRole="button" accessibilityLabel={`Request a quote for ${item.name}`} onPress={()=>router.push(`/request?serviceId=${item.id}`)} key={item.id} style={s.productCard}>{item.image_url?<Image source={{uri:item.image_url}} style={s.productImage}/>:<View style={[s.productImage,s.productImageFallback]}><Ionicons name="image-outline" size={24} color={colors.muted}/></View>}<View style={s.productBody}><View style={s.cardHeader}><Text style={s.cardTitle}>{item.name}</Text><Ionicons name="chevron-forward" size={17} color={colors.muted}/></View>{item.description&&<Text style={s.meta}>{item.description}</Text>}<Text style={s.price}>{item.delivery_mode==='REMOTE'?'🖥 Remote':item.delivery_mode==='BOTH'?'📍 Local + 🖥 Remote':'📍 Local'}{item.base_price!=null?` · From ${Number(item.base_price).toFixed(2)} AUD`:' · Quote required'}{item.duration_minutes? ` · ${item.duration_minutes} min`:''}</Text><Text style={s.cardCta}>REQUEST QUOTE</Text></Pressable>):<View style={s.emptyInline}><Text style={s.meta}>No active services listed.</Text></View>}
    <View style={s.headingRow}><Text style={s.heading}>Products</Text>{products.length?<Pressable onPress={()=>router.push(('/shop?business='+business.id+'&businessName='+encodeURIComponent(business.name)) as never)}><Text style={s.viewAll}>VIEW ALL</Text></Pressable>:null}</View>
    {products.length?<View style={s.productGrid}>{products.map(item=><Pressable accessibilityRole="button" accessibilityLabel={`View ${item.name}`} onPress={()=>router.push(`/product?id=${item.id}`)} key={item.id} style={s.card}><View style={s.cardHeader}><Text style={s.cardTitle}>{item.name}</Text><Ionicons name="chevron-forward" size={17} color={colors.muted}/></View>{item.description&&<Text style={s.meta}>{item.description}</Text>}<Text style={s.price}>${Number(item.sale_price??item.price).toFixed(2)} AUD · {item.status==='OUT_OF_STOCK'?'Out of stock':item.delivery_eligible?'Delivery available':'Pickup'}{item.pickup_available?' · Pickup available':''}</Text><Text style={s.cardCta}>VIEW PRODUCT</Text></View></Pressable>)}</View>:<View style={s.emptyInline}><Text style={s.meta}>No active products listed.</Text></View>}
