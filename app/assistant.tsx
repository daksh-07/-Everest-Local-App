...
export default function Assistant() {
  ...
  return <SafeAreaView style={s.safe}><View style={s.page}>
    ...
    <ScrollView contentContainerStyle={{paddingBottom:100}} keyboardShouldPersistTaps="handled">
      ...
      <View style={s.examples}>{['I need a car detailer this weekend','Who is Elon Musk?','I want to buy detailing products','Where is my order?'].map(x=><Pressable onPress={()=>void ask(x)} style={s.chip} key={x}><Text style={s.chipText}>{x}</Text><Ionicons name="arrow-forward" size={15} color={colors.text}/></Pressable>)}</View>
