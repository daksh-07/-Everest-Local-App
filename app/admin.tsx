...
function MfaGate({setup,onDone}:{setup:boolean;onDone:()=>void}){
 ...
 return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.page}>
   ...
   {!!diagnostic&&<View style={s.diagnosticPanel}><Text style={s.diagnostic}>Diagnostic: {diagnostic}</Text>{diagnosticDetails?.phase&&<Text style={s.diagnosticMeta}>Phase: {diagnosticDetails.phase}</Text>}{diagnosticDetails?.factorExists!==undefined&&<Text style={s.diagnosticMeta}>Factor exists: {diagnosticDetails.factorExists?'yes':'no'}</Text>}{diagnosticDetails?.factorStatus&&<Text style={s.diagnosticMeta}>Factor status: {diagnosticDetails.factorStatus}</Text>}{diagnosticDetails?.factorType&&<Text style={s.diagnosticMeta}>Factor type: {diagnosticDetails.factorType}</Text>}{diagnosticDetails?.challengeCreated!==undefined&&<Text style={s.diagnosticMeta}>Challenge created: {diagnosticDetails.challengeCreated?'yes':'no'}</Text>}{diagnosticDetails?.challengeIdExists!==undefined&&<Text style={s.diagnosticMeta}>Challenge ID present: {diagnosticDetails.challengeIdExists?'yes':'no'}</Text>}{diagnosticDetails?.code&&<Text style={s.diagnosticMeta}>Supabase error code: {diagnosticDetails.code}</Text>}{diagnosticDetails?.status!==undefined&&<Text style={s.diagnosticMeta}>HTTP status: {diagnosticDetails.status}</Text>}{diagnosticDetails?.message&&<Text selectable style={s.diagnosticMessage}>Supabase message: {diagnosticDetails.message}</Text>}</View>}
