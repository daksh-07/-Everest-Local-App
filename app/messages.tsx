...
export default function Messages() {
  ...
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.page}><Text style={s.eyebrow}>EVEREST LOCAL</Text><Text style={s.title}>Messages</Text><Text style={s.copy}>Conversations are only visible to their authorized participants.</Text>{loading ? <ActivityIndicator/> : error ? <View style={s.errorBox}><Text style={s.error}>{error}</Text><Pressable onPress={() => void loadList()} disabled={loading} style={s.retry}><Text style={s.retryText}>{loading ? 'RETRYING…' : 'RETRY'}</Text></Pressable></View> : items.length ? items.map(item => <Pressable key={item.id} onPress={() => setSelected(item)} style={s.card}><Text style={s.cardTitle}>Marketplace conversation</Text><Text style={s.meta}>{new Date(item.created_at).toLocaleString()}</Text></Pressable>) : <View style={s.empty}><Text style={s.emptyTitle}>No messages yet.</Text><Text style={s.meta}>Messages will appear here after a real marketplace interaction.</Text></View>}</ScrollView></SafeAreaView>;
}
