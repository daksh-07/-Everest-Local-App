...
export default function Request(){
 ...
 const [taxonomyLoading,setTaxonomyLoading]=useState(!serviceId);const [serviceDefinitions,setServiceDefinitions]=useState<ServiceDefinition[]>([]);const [taxonomyCategories,setTaxonomyCategories]=useState<TaxonomyCategory[]>([]);const [serviceQuery,setServiceQuery]=useState('');const [selectedDefinition,setSelectedDefinition]=useState<ServiceDefinition|null>(null);
 ...
 const categoryById=useMemo(()=>Object.fromEntries(taxonomyCategories.map(x=>[x.id,x])),[taxonomyCategories]);
 const visibleDefinitions=useMemo(()=>{const q=serviceQuery.trim().toLowerCase();const source=q?serviceDefinitions.filter(item=>item.name.toLowerCase().includes(q)||(item.description??'').toLowerCase().includes(q)||(categoryById[item.category_id]?.name??'').toLowerCase().includes(q)):serviceDefinitions;return source.slice(0,40)},[serviceQuery,serviceDefinitions,categoryById]);

 useEffect(()=>{if(serviceId){setTaxonomyLoading(false);return;}let active=true;(async()=>{try{const taxonomy=await listServiceTaxonomy();if(!active)return;setServiceDefinitions(taxonomy.services);setTaxonomyCategories([...taxonomy.roots,...taxonomy.subcategories]);}catch{if(active)setError('Service types could not be loaded. Please retry.')}finally{if(active)setTaxonomyLoading(false)}})();return()=>{active=false}},[serviceId]);
 ...
 async function submit(){if(submitRef.current||busy||!validate(2))return;submitRef.current=true;setBusy(true);setError('');
  try{
   ...
   const id=await createServiceRequest({categoryId:selectedDefinition?.category_id,serviceDefinitionId:selectedDefinition?.id,serviceId:serviceId||undefined,description:description.trim(),deliveryMode:selectedDefinition?.default_delivery_mode==='BOTH'?effectiveMode:(selectedDefinition?.default_delivery_mode??effectiveMode),
 ...
 return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.page} keyboardShouldPersistTaps="handled">
  ...
  {step===1?<><Text style={s.title}>What service do you need?</Text><Text style={s.copy}>Choose the exact service type first. Everest uses this to find businesses that actually offer that service.</Text>
   {!serviceId?<View style={s.nicheSection}>
    <TextInput value={serviceQuery} onChangeText={setServiceQuery} placeholder="Search services — detailing, plumber, cleaner, web design…" placeholderTextColor={colors.muted} style={s.searchInput}/>
    {taxonomyLoading?<ActivityIndicator color={colors.brand} style={{marginVertical:18}}/>:selectedDefinition?<View style={s.selectedNiche}><View style={{flex:1}}><Text style={s.eyebrow}>SELECTED SERVICE TYPE</Text><Text style={s.nicheName}>{selectedDefinition.name}</Text><Text style={s.meta}>{(categoryById[selectedDefinition.category_id]?.name??'Service')+' · '+(selectedDefinition.default_delivery_mode==='BOTH'?'Local or remote':selectedDefinition.default_delivery_mode==='REMOTE'?'Remote':'Local')}</Text></View><Pressable onPress={()=>{setSelectedDefinition(null);setServiceQuery('')}} style={s.changeNiche}><Text style={s.changeNicheText}>CHANGE</Text></Pressable></View>:<View style={s.nicheResults}>{visibleDefinitions.map(item=><Pressable key={item.id} onPress={()=>{setSelectedDefinition(item);setServiceQuery(item.name);if(item.default_delivery_mode==='REMOTE')setMode('REMOTE');else if(item.default_delivery_mode==='LOCAL')setMode('LOCAL')}} style={s.nicheCard}><View style={{flex:1}}><Text style={s.nicheName}>{item.name}</Text><Text numberOfLines={1} style={s.meta}>{(categoryById[item.category_id]?.name??'Service')+(item.description?' · '+item.description:'')}</Text></View><Ionicons name="chevron-forward" size={18} color={colors.muted}/></Pressable>)}</View>}
   </View>:null}
   <Text style={s.heading}>Job details</Text><TextInput value={description} onChangeText={setDescription} placeholder="Example: Full interior + exterior car detail this Saturday" placeholderTextColor={colors.muted} multiline maxLength={5000} style={[s.input,s.area]}/>
  ...
  {step===3?<><Text style={s.title}>Ready to post?</Text><Text style={s.copy}>Tap any row to edit it before Everest starts matching.</Text>
   <Review label="SERVICE TYPE" value={serviceName||selectedDefinition?.name||'Not selected'} onPress={()=>setStep(1)} colors={colors}/>
   <Review label="JOB DETAILS" value={description} onPress={()=>setStep(1)} colors={colors}/>
