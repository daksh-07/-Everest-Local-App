...
type Priority={kind:'LEAD'|'FOLLOW_UP'|'JOB'|'ORDER';title:string;subtitle:string;route:'/business-leads'|'/business-customers'|'/business-jobs'|'/business-control'};
type Snapshot={leads:number;crmLeads:number;followUps:number;quotes:number;jobsToday:number;unread:number;orders:number;revenue:number|null;completed:number};
...
export default function BusinessToday(){
 ...
 const [snap,setSnap]=useState<Snapshot>({leads:0,crmLeads:0,followUps:0,quotes:0,jobsToday:0,unread:0,orders:0,revenue:null,completed:0});
 ...
 const load=useCallback(async(refresh=false)=>{if(refresh)setRefreshing(true);else setLoading(true);setError('');
  try{
   ...
   if(current.verification_status!=='VERIFIED'){
    setSnap({leads:0,crmLeads:0,followUps:0,quotes:0,jobsToday:0,unread:0,orders:0,revenue:null,completed:0});setPriority(null);return;
   }
   ...
   const [leadR,crmLeadR,followUpR,quoteR,jobR,orderR,convR,payoutR,completedR]=await Promise.all([
    ...
    supabase.from('business_contacts').select('id',{count:'exact',head:true}).eq('business_id',id).in('lead_status',['NEW_LEAD','CONTACTED','QUALIFIED','QUOTE_SENT','FOLLOW_UP']),
    supabase.from('crm_tasks').select('id,task,due_at',{count:'exact'}).eq('business_id',id).eq('status','OPEN').lte('due_at',endToday.toISOString()).order('due_at',{ascending:true}).limit(1),
   ...
   for(const result of [leadR,crmLeadR,followUpR,quoteR,jobR,orderR,convR,payoutR,completedR])if(result.error)throw result.error;
   ...
   setSnap({leads:leadR.count??0,crmLeads:crmLeadR.count??0,followUps:followUpR.count??0,quotes:quoteR.count??0,jobsToday:jobR.count??0,orders:orderR.count??0,unread,revenue,completed:completedR.count??0});
   const lead=(leadR.data??[])[0] as LeadRow|undefined;const followUp=(followUpR.data??[])[0] as {task:string;due_at:string}|undefined;const job=(jobR.data??[])[0] as JobRow|undefined;const order=(orderR.data??[])[0] as OrderRow|undefined;
   ...
   else if(followUp)setPriority({kind:'FOLLOW_UP',title:followUp.task,subtitle:'Due '+new Date(followUp.due_at).toLocaleString(),route:'/business-customers'});
 ...
 const cards=[['Marketplace leads',snap.leads,'/business-leads'],['CRM leads',snap.crmLeads,'/business-customers'],['Follow-ups due',snap.followUps,'/business-customers'],['Quotes',snap.quotes,'/business-leads'],['Today',snap.jobsToday,'/business-jobs'],['Messages',snap.unread,'/business-inbox'],['Orders',snap.orders,'/business-control']] as const;
 ...
 return <SafeAreaView style={st.safe} edges={['top']}><View style={{flex:1}}><ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>void load(true)} tintColor={colors.brand}/>} contentContainerStyle={st.page}>
  ...
  {loading?<ActivityIndicator color={colors.brand} style={{marginTop:60}}/>:<>
   ...
   <View style={st.section}><Text style={st.sectionLabel}>QUICK ACTIONS</Text><View style={st.actions}><Quick label="Add customer" onPress={()=>router.push('/business-customers')} colors={colors}/><Quick label="Create post" onPress={()=>router.push('/create-post')} colors={colors}/><Quick label="Update services" onPress={()=>router.push('/services')} colors={colors}/><Quick label="Add product" onPress={()=>router.push('/products')} colors={colors}/><Quick label="View profile" onPress={()=>business&&router.push({pathname:'/business-profile',params:{id:business.id}})} colors={colors}/><Quick label="Service area" onPress={()=>router.push('/service-areas')} colors={colors}/></View></View></>:null}
