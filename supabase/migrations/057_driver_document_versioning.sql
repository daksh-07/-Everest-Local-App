-- Verified evidence is never overwritten. A new upload creates a new version
-- and retains the prior row for audit history.

alter table public.driver_documents
  add column if not exists supersedes_document_id uuid references public.driver_documents(id);

drop function if exists public.register_driver_document(uuid,uuid,text,text,text,bigint,date,text,text,text);
...
as $$
declare did uuid; existing_id uuid; existing_status text;
begin
  ...
  select id,status into existing_id,existing_status from public.driver_documents
  ...
  if existing_id is not null and existing_status<>'VERIFIED' then
    update public.driver_documents set status='REPLACED' where id=existing_id;
  end if;
  ...
  insert into public.driver_documents(
    ...
    expires_at,document_number,issuing_jurisdiction,supersedes_document_id
  )
  values(
    ...
    nullif(trim(p_document_number),''),upper(nullif(trim(p_issuing_jurisdiction),'')),existing_id
  ...
  values(p_application_id,auth.uid(),'DOCUMENT_UPLOADED',
    ...
    jsonb_build_object(
      'document_type',p_document_type,'document_subtype',p_document_subtype,
      'supersedes_document_id',existing_id,'prior_status',existing_status
    ));
  return did;
end;
$$;
