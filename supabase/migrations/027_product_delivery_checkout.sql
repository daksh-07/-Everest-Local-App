...
-- Delivery eligibility and business publication state are validated from trusted rows inside the locked checkout transaction.
...
begin
  ...
  for item in
    select ci.id as cart_item_id,ci.product_id,ci.quantity,p.name,p.price,p.sale_price,p.status,p.business_id,p.delivery_eligible,p.pickup_available,b.status as business_status,b.verification_status,b.accepts_orders
    from public.cart_items ci join public.products p on p.id=ci.product_id join public.businesses b on b.id=p.business_id
  ...
  for item in select * from _checkout_items loop
    ...
    insert into public.order_items(order_id,product_id,product_name,unit_price,quantity,line_total,source_cart_item_id) values(oid,item.product_id,item.name,item.unit_price,item.quantity,item.unit_price*item.quantity,item.cart_item_id);
  end loop;
