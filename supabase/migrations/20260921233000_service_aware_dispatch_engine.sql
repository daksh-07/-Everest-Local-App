-- Additive service-aware dispatch layer. Existing quote, booking, and driver-delivery flows remain unchanged.
-- Provider identity is a verified business member; capabilities are explicit per business service.
...
create table if not exists public.service_dispatch_config (
  ...
  default_provider_radius_km numeric(5,2) not null default 15 check (default_provider_radius_km between 1 and 100),
...
create table if not exists public.service_dispatch_jobs (
  ...
  status text not null default 'SEARCHING' check (status in ('SEARCHING','PROVIDER_OFFERED','ACCEPTED','ASSIGNED','IN_PROGRESS','COMPLETED','CANCELLED','EXHAUSTED')),
...
create unique index if not exists service_dispatch_one_live_offer_per_provider_idx
  on public.service_dispatch_offers(job_id,provider_id) where status='OFFERED';
create index if not exists service_dispatch_offers_job_rank_idx
  on public.service_dispatch_offers(job_id,status,eta_seconds,location_age_seconds,compatibility_score);
create index if not exists service_dispatch_jobs_status_idx
  on public.service_dispatch_jobs(status,updated_at desc);
create index if not exists service_provider_profiles_available_idx
  on public.service_provider_profiles(availability_status,updated_at desc) where availability_status='AVAILABLE';
create index if not exists service_provider_capabilities_service_idx
  on public.service_provider_capabilities(service_id,provider_id);
create unique index if not exists service_dispatch_one_active_assignment_per_provider_idx
  on public.service_dispatch_assignments(provider_id) where completed_at is null;
...
revoke all on public.service_dispatch_config from anon,authenticated;
revoke all on public.service_provider_profiles from anon,authenticated;
revoke all on public.service_provider_capabilities from anon,authenticated;
revoke all on public.service_provider_locations from anon,authenticated;
revoke all on public.service_dispatch_jobs from anon,authenticated;
revoke all on public.service_dispatch_offers from anon,authenticated;
revoke all on public.service_dispatch_assignments from anon,authenticated;
...
begin
  ...
  where id=p_request_id and customer_id=auth.uid() and status not in ('BOOKED','COMPLETED','CANCELLED');
...
as $$
declare uid uuid:=auth.uid(); cfg public.service_dispatch_config; row public.service_provider_profiles;
begin
  ...
  if p_service_radius_km is null or p_service_radius_km < 1 or p_service_radius_km > 100 then raise exception 'Invalid service radius'; end if;
...
returns boolean language plpgsql security definer set search_path to 'public'
as $$
declare uid uuid:=auth.uid(); cfg public.service_dispatch_config; previous public.service_provider_locations;
begin
  if uid is null then raise exception 'Authentication required'; end if;
  ...
  if p_latitude is null or p_longitude is null or p_recorded_at is null then raise exception 'Location data is incomplete'; end if;
  if p_latitude < -90 or p_latitude > 90 or p_recorded_at > now()+interval '2 minutes' then raise exception 'Invalid location'; end if;
  if p_longitude < -180 or p_longitude > 180 then raise exception 'Invalid location'; end if;
  select * into cfg from public.service_dispatch_config where id=true;
...
as $$
  with job as (
    select j.id,j.request_id,j.status,r.service_id,r.category_id,r.suburb,r.city,r.state,r.latitude,r.longitude
    ...
    select p.provider_id,p.business_id,
      greatest(0,round((earth_distance(ll_to_earth(pl.latitude,pl.longitude),ll_to_earth(j.latitude,j.longitude))/1000.0)/nullif(p.travel_speed_kmh,0)*3600))::int as eta_seconds,
      greatest(0,extract(epoch from (now()-pl.recorded_at)))::int as location_age_seconds,
      case when j.service_id is not null and exists(select 1 from public.service_provider_capabilities pc where pc.provider_id=p.provider_id and pc.service_id=j.service_id) then 100
           when j.category_id is not null and exists(select 1 from public.service_provider_capabilities pc join public.services s on s.id=pc.service_id where pc.provider_id=p.provider_id and s.category_id=j.category_id and s.active) then 80 else 0 end as compatibility_score,
      (select s.base_price from public.services s where s.business_id=p.business_id and s.active and (j.service_id is null or s.id=j.service_id) order by s.base_price nulls last limit 1) as expected_job_value,
      (select s.duration_minutes from public.services s where s.business_id=p.business_id and s.active and (j.service_id is null or s.id=j.service_id) order by s.duration_minutes desc nulls last limit 1) as estimated_duration_minutes,
      earth_distance(ll_to_earth(pl.latitude,pl.longitude),ll_to_earth(j.latitude,j.longitude))/1000.0 as distance_km
    ...
    where p.availability_status='AVAILABLE'
      ...
      and (j.service_id is not null and exists(select 1 from public.service_provider_capabilities pc where pc.provider_id=p.provider_id and pc.service_id=j.service_id)
           or j.service_id is null and (j.category_id is null or exists(select 1 from public.service_provider_capabilities pc join public.services s on s.id=pc.service_id where pc.provider_id=p.provider_id and s.category_id=j.category_id and s.active)))
  ...
  order by eta_seconds asc,location_age_seconds asc,compatibility_score desc,provider_id asc
...
begin
  ...
  where not exists(select 1 from public.service_dispatch_offers o where o.job_id=p_job_id and o.provider_id=c.provider_id and o.status in ('OFFERED','ACCEPTED','DECLINED'))
  order by c.eta_seconds asc,c.location_age_seconds asc,c.compatibility_score desc,c.provider_id asc limit 1;
  if c.provider_id is null then
    update public.service_dispatch_jobs set status='EXHAUSTED',updated_at=now() where id=p_job_id;
    insert into public.audit_logs(actor_id,action,entity_type,entity_id,metadata) values(auth.uid(),'SERVICE_DISPATCH_EXHAUSTED','SERVICE_DISPATCH_JOB',p_job_id,'{}'::jsonb);
    return null;
  end if;
  ...
  update public.service_dispatch_jobs set status='PROVIDER_OFFERED',updated_at=now(),estimated_duration_minutes=coalesce(j.estimated_duration_minutes,c.estimated_duration_minutes) where id=p_job_id;
  insert into public.audit_logs(actor_id,action,entity_type,entity_id,metadata) values(auth.uid(),'SERVICE_PROVIDER_CONSIDERED','SERVICE_DISPATCH_JOB',p_job_id,jsonb_build_object('provider_id',c.provider_id,'eta_seconds',c.eta_seconds,'location_age_seconds',c.location_age_seconds,'compatibility_score',c.compatibility_score));
  insert into public.audit_logs(actor_id,action,entity_type,entity_id,metadata) values(auth.uid(),'SERVICE_DISPATCH_OFFER_CREATED','SERVICE_DISPATCH_OFFER',oid,jsonb_build_object('job_id',p_job_id,'provider_id',c.provider_id,'eta_seconds',c.eta_seconds));
...
returns uuid language plpgsql security definer set search_path to 'public'
as $$
declare r public.service_requests; j public.service_dispatch_jobs; oid uuid;
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  select * into r from public.service_requests where id=p_request_id for update;
  if r.id is null or r.customer_id<>auth.uid() then raise exception 'Not authorized'; end if;
  ...
  insert into public.service_dispatch_jobs(request_id,status,scheduled_start_at,created_at,updated_at)
  values(r.id,'SEARCHING',now(),now(),now())
  ...
  returning * into j;
  insert into public.audit_logs(actor_id,action,entity_type,entity_id,metadata) values(auth.uid(),'SERVICE_DISPATCH_STARTED','SERVICE_DISPATCH_JOB',j.id,jsonb_build_object('request_id',r.id));
  oid:=public.dispatch_next_service_provider(j.id);
  return j.id;
end;
$$;
...
begin
  ...
  if o.status<>'OFFERED' then raise exception 'Job no longer available'; end if;
  ...
  if not exists(select 1 from public.service_provider_profiles p where p.provider_id=auth.uid() and p.availability_status='AVAILABLE') then raise exception 'Provider is no longer available'; end if;
  if exists(select 1 from public.service_dispatch_assignments a where a.provider_id=auth.uid() and a.completed_at is null) then raise exception 'Provider already has an active assignment'; end if;
  update public.service_dispatch_jobs set status='ASSIGNED',claimed_provider_id=auth.uid(),claimed_at=now(),updated_at=now() where id=j.id and status='PROVIDER_OFFERED';
  if not found then
    update public.service_dispatch_offers set status='REJECTED',responded_at=now() where id=o.id and status='OFFERED';
    insert into public.audit_logs(actor_id,action,entity_type,entity_id,metadata) values(auth.uid(),'SERVICE_DISPATCH_ASSIGNMENT_REJECTED_ALREADY_CLAIMED','SERVICE_DISPATCH_JOB',j.id,jsonb_build_object('offer_id',o.id));
    raise exception 'Job no longer available';
  end if;
  insert into public.service_dispatch_assignments(job_id,provider_id,scheduled_start_at,scheduled_end_at)
  values(j.id,auth.uid(),j.scheduled_start_at,case when j.estimated_duration_minutes is null then null else j.scheduled_start_at+make_interval(mins=>j.estimated_duration_minutes) end);
  ...
  insert into public.audit_logs(actor_id,action,entity_type,entity_id,metadata) values(auth.uid(),'SERVICE_DISPATCH_ASSIGNMENT_WON','SERVICE_DISPATCH_JOB',j.id,jsonb_build_object('provider_id',auth.uid(),'request_id',r.id));
  ...
  return jsonb_build_object('accepted',true,'job_id',j.id,'provider_id',auth.uid());
...
as $$
declare job_row record; expired_count integer:=0;
begin
  for job_row in select distinct j.id from public.service_dispatch_jobs j left join public.service_dispatch_offers o on o.job_id=j.id and o.status='OFFERED' where j.status in ('SEARCHING','PROVIDER_OFFERED') and (o.id is null or o.expires_at<=now()) loop
...
revoke all on function public.set_service_request_location(uuid,numeric,numeric) from public,anon,authenticated;
grant execute on function public.set_service_request_location(uuid,numeric,numeric) to authenticated;
revoke all on function public.set_service_provider_profile(uuid,text,numeric,numeric) from public,anon,authenticated;
grant execute on function public.set_service_provider_profile(uuid,text,numeric,numeric) to authenticated;
revoke all on function public.update_service_provider_location(numeric,numeric,numeric,timestamptz) from public,anon,authenticated;
grant execute on function public.update_service_provider_location(numeric,numeric,numeric,timestamptz) to authenticated;
revoke all on function public.set_service_provider_capability(uuid,boolean) from public,anon,authenticated;
...
revoke all on function public.start_service_dispatch(uuid) from public,anon,authenticated;
grant execute on function public.start_service_dispatch(uuid) to authenticated;
revoke all on function public.respond_service_dispatch_offer(uuid,text) from public,anon,authenticated;
...
select cron.schedule('service-dispatch-queue','* * * * *','select public.process_service_dispatch_queue();')
where not exists (select 1 from cron.job where jobname='service-dispatch-queue');

-- Provider/customer visibility is intentionally limited. Offers expose only provider-scoped data; dispatch jobs expose only customer or claimed-provider rows.
drop policy if exists service_provider_profiles_self on public.service_provider_profiles;
...
drop policy if exists service_dispatch_jobs_participant on public.service_dispatch_jobs;
create policy service_dispatch_jobs_participant on public.service_dispatch_jobs for select to authenticated using (exists(select 1 from public.service_requests r where r.id=request_id and r.customer_id=(select auth.uid())) or claimed_provider_id=(select auth.uid()));
...
drop policy if exists service_dispatch_assignments_participant on public.service_dispatch_assignments;
create policy service_dispatch_assignments_participant on public.service_dispatch_assignments for select to authenticated using (provider_id=(select auth.uid()));
