...
Deno.serve(async req=>{
 ...
 if(req.method==='GET'){
  ...
  if(state.provider==='GMAIL'){const {data:subscription}=await admin.from('business_subscriptions').select('status').eq('business_id',state.businessId).maybeSingle();if(!subscription||!['ACTIVE','TRIALING'].includes(String(subscription.status)))return json({error:'Everest Pro is required to connect Gmail.'},402);}
 ...
 if(action==='BEGIN'){
  const provider=String(body.provider??'') as Provider;if(!providerSet.has(provider))return json({error:'Unsupported provider.'},400);if(provider==='GMAIL'){const {data:subscription}=await userClient.from('business_subscriptions').select('status').eq('business_id',businessId).maybeSingle();if(!subscription||!['ACTIVE','TRIALING'].includes(String(subscription.status)))return json({error:'Everest Pro is required to connect Gmail.'},402);}const cfg=config(provider);if(!cfg.clientId||!cfg.clientSecret||!cfg.callback)return json({error:'Provider OAuth credentials are not configured.'},503);
