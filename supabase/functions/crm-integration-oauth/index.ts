...
type CustomerCalendarState={kind:'CUSTOMER_CALENDAR';userId:string;nonce:string;exp:number};
type OAuthState=BusinessState|CustomerCalendarState;
...
async function signedState(state:BusinessState,secret:string){
 const body=b64u(enc.encode(JSON.stringify(state)));
...
async function verifyState(value:string,secret:string):Promise<OAuthState|null>{
 ...
 const state=JSON.parse(new TextDecoder().decode(fromB64u(body))) as OAuthState;
 if(!state||typeof state!=='object'||typeof state.userId!=='string'||typeof state.exp!=='number'||state.exp<=Date.now())return null;
 if('kind' in state)return state.kind==='CUSTOMER_CALENDAR'?state:null;
 return providerSet.has(state.provider)?state:null;
}
...
async function encryptTokens(tokens:unknown,secret:string){
 const iv=crypto.getRandomValues(new Uint8Array(12));
 const key=await encryptionKey(secret);
 const cipher=await crypto.subtle.encrypt({name:'AES-GCM',iv},key,enc.encode(JSON.stringify(tokens)));
 return{ciphertext:b64u(new Uint8Array(cipher)),iv:b64u(iv)};
}

function config(provider:Provider,callback:string){
 if(provider==='MICROSOFT_CALENDAR')return{
...
Deno.serve(async req=>{
 ...
 if(!url||!publishable||!secretKey||!stateSecret||!tokenSecret||!callback)return json({error:'Integration service is not configured.'},503);
 ...
 if(req.method==='GET'){
  ...
  if('kind' in state){
   ...
   const tokenResponse=await fetch('https://oauth2.googleapis.com/token',{
    ...
    body:new URLSearchParams({client_id:clientId,client_secret:clientSecret,code,redirect_uri:callback,grant_type:'authorization_code'})
   });
   ...
   await admin.from('audit_logs').insert({
    actor_id:state.userId,action:'CUSTOMER_CALENDAR_CONNECTED',entity_type:'customer_calendar_connection',entity_id:connectionId,metadata:{provider:'GOOGLE_CALENDAR'}
   });
   return Response.redirect(appUrl+'/customer-calendar?connected=google',302);
  }

  const cfg=config(state.provider,callback);
  if(!cfg.clientId||!cfg.clientSecret)return json({error:'Provider credentials are not configured.'},503);
  const tokenResponse=await fetch(cfg.token,{
   method:'POST',
   headers:{'Content-Type':'application/x-www-form-urlencoded'},
   body:new URLSearchParams({client_id:cfg.clientId,client_secret:cfg.clientSecret,code,redirect_uri:cfg.callback,grant_type:'authorization_code'})
  });
  if(!tokenResponse.ok)return json({error:'Provider token exchange failed.'},502);
  const tokens=await tokenResponse.json();

  const profileUrl=state.provider==='MICROSOFT_CALENDAR'?'https://graph.microsoft.com/v1.0/me':'https://openidconnect.googleapis.com/v1/userinfo';
  const profileResponse=await fetch(profileUrl,{headers:{Authorization:'Bearer '+tokens.access_token}});
  if(!profileResponse.ok)return json({error:'Provider account lookup failed.'},502);
  const profile=await profileResponse.json();
  ...
  if(state.provider==='GMAIL'){
   ...
   if(!subscription||!['ACTIVE','TRIALING'].includes(String(subscription.status)))return json({error:'Everest Pro is required to connect Gmail.'},402);
  }

  const {data:existing}=await admin.from('integration_connections').select('id').eq('business_id',state.businessId).eq('provider',state.provider).eq('provider_account_id',accountId).maybeSingle();
  const connectionId=existing?.id??crypto.randomUUID();
  const {error:connectionError}=await admin.from('integration_connections').upsert({
   id:connectionId,business_id:state.businessId,connected_by_user_id:state.userId,provider:state.provider,provider_account_id:accountId,account_label:label,status:'CONNECTED',
   granted_scopes:String(tokens.scope??'').split(' ').filter(Boolean),secret_reference:'integration_oauth_credentials/'+connectionId,last_error_code:null,
   connected_at:new Date().toISOString(),revoked_at:null,updated_at:new Date().toISOString()
  },{onConflict:'id'});
  if(connectionError)return json({error:'Connection could not be saved.'},500);

  const encrypted=await encryptTokens(tokens,tokenSecret);
  const expiresAt=tokens.expires_in?new Date(Date.now()+Number(tokens.expires_in)*1000).toISOString():null;
  const {error:credentialError}=await admin.from('integration_oauth_credentials').upsert({
   connection_id:connectionId,business_id:state.businessId,token_ciphertext:encrypted.ciphertext,token_iv:encrypted.iv,token_expires_at:expiresAt,updated_at:new Date().toISOString()
  },{onConflict:'connection_id'});
  if(credentialError)return json({error:'Credentials could not be stored.'},500);

  if(state.provider!=='GMAIL')await admin.from('calendar_connections').upsert({
   id:connectionId,business_id:state.businessId,provider:state.provider,calendar_label:'Primary calendar',sync_enabled:true
  },{onConflict:'id'});

  await admin.from('audit_logs').insert({
   actor_id:state.userId,action:'INTEGRATION_CONNECTED',entity_type:'integration_connection',entity_id:connectionId,metadata:{business_id:state.businessId,provider:state.provider}
  });
 ...
 if(action==='BEGIN'){
  ...
  if(!providerSet.has(provider))return json({error:'Unsupported provider.'},400);
  if(provider==='GMAIL'){
   const {data:subscription}=await userClient.from('business_subscriptions').select('status').eq('business_id',businessId).maybeSingle();
   if(!subscription||!['ACTIVE','TRIALING'].includes(String(subscription.status)))return json({error:'Everest Pro is required to connect Gmail.'},402);
  }
  const cfg=config(provider,callback);
  if(!cfg.clientId||!cfg.clientSecret)return json({error:'Provider OAuth credentials are not configured.'},503);
  const state=await signedState({businessId,userId:user.id,provider,nonce:crypto.randomUUID(),exp:Date.now()+10*60*1000},stateSecret);
  const params=new URLSearchParams({client_id:cfg.clientId,redirect_uri:cfg.callback,response_type:'code',scope:cfg.scopes.join(' '),state});
  if(provider!=='MICROSOFT_CALENDAR'){
   params.set('access_type','offline');
   params.set('prompt','consent');
   params.set('include_granted_scopes','true');
  }
  return json({authorizationUrl:cfg.authorize+'?'+params});
 }

 if(action==='DISCONNECT'){
  const connectionId=String(body.connectionId??'');
  const {data:connection}=await admin.from('integration_connections').select('id,provider').eq('id',connectionId).eq('business_id',businessId).maybeSingle();
  if(!connection)return json({error:'Connection not found.'},404);
  await admin.from('integration_oauth_credentials').delete().eq('connection_id',connectionId).eq('business_id',businessId);
  await admin.from('integration_connections').update({status:'REVOKED',revoked_at:new Date().toISOString(),secret_reference:null,updated_at:new Date().toISOString()}).eq('id',connectionId).eq('business_id',businessId);
  await admin.from('audit_logs').insert({
   actor_id:user.id,action:'INTEGRATION_DISCONNECTED',entity_type:'integration_connection',entity_id:connectionId,metadata:{business_id:businessId,provider:connection.provider}
  });
