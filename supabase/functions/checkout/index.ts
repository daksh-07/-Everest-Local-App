...
Deno.serve(async req=>{
 ...
 try{
  const {data:rawOrder,error}=await userClient.rpc('create_order_from_cart',{p_idempotency_key:idem,p_delivery_method:'PICKUP',p_delivery_address:null});
  if(error)throw error;
  const order=rawOrder as OrderResult;
