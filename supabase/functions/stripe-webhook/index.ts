...
Deno.serve(async req=>{
 ...
 try{
  ...
  if(event.type==='checkout.session.completed'||event.type==='payment_intent.succeeded'){
   ...
   if((changed??[]).length>0){
    ...
    for(const item of items??[]){const {error}=await db.rpc('finalize_inventory_sale',{p_product_id:item.product_id,p_quantity:item.quantity});if(error)throw error;}
   }
 ...
 }catch(error){console.error('stripe_webhook_failed',{message:error instanceof Error?error.message:'unknown'});return json({error:'Webhook processing failed'},500)}
});
